server <- function(input, output) {
  ...
  # Reactive to indicate that model has run
  isRun <- reactiveValues( yes = FALSE )
  modelStats <- reactiveValues( nEQ = 0,
                                nMax = 0,
                                mMax = 0,
                                mMin = 0,
                                longest = 0,
  ...
  output$plot1 <- renderPlot({
    ...
    if ( is.na( rseed ) | is.null( rseed ) ) rseed <- 12345678
    set.seed( rseed )
    n <- input$n_segments
    ...
    ghist <- ggplot( df, aes( A ) ) +
      geom_histogram( bins = floor( 15 * n / 50 ), fill = "lightsteelblue", color = "black" ) +
  ...
  output$plot3 <- renderPlot({
    ...
    # Fault slip plot
    pal = colorRampPalette( c( "white", "grey90", "grey85",
                               "grey80", "khaki", "orange",
                               "steelblue3" ), bias = 1, space = "rgb" )(256)
    par( mar = c( 1, 4, 4, 1 ) )
    image( x = dt[-1],
           y = y,
           z  = log10( 0.01 + diff(dX) ),
           col = pal,
           axes = FALSE,
           ann = FALSE )
    axis( 3 )
  ...
  output$table <- renderTable({
    if ( !isRun$yes ) hide( "table" )
    else show( "table" )

    snames <- c( "Number of earthquakes",
                 "Maximum magnitude",
                 "Minimum magnitude",
    ...
    snums <- c( as.integer( modelStats$nEQ ),
    ...
    for ( i in 1:nt ) {
      ...
      if ( any( M1 > 0 ) ) {
        ...
        for ( j in seq_along( l ) ) {
          ...
          k <- k + 1
          longest <- max( longest, sum( A[seq(ends[l[j]] + 1, ends[l[j]+1] )] ) * fLength )
        }
      }
    }
    mag <- mag[mag > 0]
    ...
    for( i in 1:n ) {
      ...
      if ( any( M1 ) ) {
        ...
        for ( j in seq_along( l ) ) {
          ...
          gapShortest <- min( gapShortest, ends[l[j]+1] - ends[l[j]] + 1 )
        }
      }
    }
  ...
  observeEvent(input$runIt, {
    isRun$yes <- FALSE
    x <<- runSimulation( input$fault_dist,
                         input$n_segments,
                         input$kexp,
                         input$static_base,
                         input$random_seed,
